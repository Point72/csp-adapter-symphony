...
To work on `csp-adapter-symphony`, you are going to need to build it from source. See
[Build from Source](Build-from-Source) for
detailed build instructions.

Once you've built `csp-adapter-symphony` from a `git` clone, you will also need to
configure `git` and your GitHub account for `csp-adapter-symphony` development.

## Step 2: Configuring Git and GitHub for Development

### Create your fork

The first step is to create a personal fork of `csp-adapter-symphony`. To do so, click
the "fork" button at https://github.com/Point72/csp-adapter-symphony, or just navigate
[here](https://github.com/Point72/csp-adapter-symphony/fork) in your browser. Set the
